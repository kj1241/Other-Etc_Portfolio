...
public:
    Practice10FrameListener(Ogre::SceneNode* node, Ogre::Entity* ent, Ogre::SceneNode* node2, Ogre::Entity* ent2, Ogre::SceneNode* camNode, Ogre::Camera* cam)
        : _node(node), _ent(ent), _node2(node2), _ent2(ent2), _camNode(camNode), _cam(cam), _movementspeed(200.0f), _pressedBefore(false),
        _PolyMode(Ogre::PolygonMode::PM_SOLID), _moveForward(false), _moveBackward(false),
        _moveLeft(false), _moveRight(false), _translateVector(Ogre::Vector3::ZERO)
    {
        _aniStateIdle = _ent->getAnimationState("IdleBase");
    ...
    bool frameStarted(const Ogre::FrameEvent& evt) override
    {
        ...
        Ogre::AxisAlignedBox boundingBox2 = _ent2->getWorldBoundingBox();

        if (boundingBox1.intersects(boundingBox2))
        {
            Ogre::Vector3 direction = _node2->getPosition() - _node->getPosition();
            direction.normalise();

            newPosition = _node->getPosition() - direction * 10.0f; 
        }
...
public:
    ...
    void setup() override
    {
        ...
        _SinbadNode2->setPosition(Ogre::Vector3(100.0f, 4.0f, 0.0f));  // Position the second Sinbad at a different location
        ...
        Practice10FrameListener* frameListener = new Practice10FrameListener(_SinbadNode, _SinbadEnt, _SinbadNode2, _SinbadEnt2, camNode, cam);
        root->addFrameListener(frameListener);
        addInputListener(frameListener);
    }
};
